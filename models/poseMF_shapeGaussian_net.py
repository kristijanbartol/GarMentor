...
class PoseMFShapeGaussianNet(nn.Module):
    def __init__(self,
        ...
        self.use_style = config.MODEL.USE_STYLE
        self.classify_garment = config.MODEL.CLASSIFY_GARMENT
        ...
        self.fc_shape = nn.Linear(
            fc1_dim, 
            self.num_shape_params * 2
        ) # x2 because of (means, variances)
        self.fc_style = nn.Linear(
            fc1_dim, 
            self.num_style_params * self.num_garment_classes * 2      
        ) # xN for garment classes, x2 for (means, variances)

        self.fc_glob = nn.Linear(fc1_dim, self.num_glob_params)
        self.fc_cam = nn.Linear(fc1_dim, self.num_cam_params)

        self.fc_upper_binary_classifier = nn.Linear(fc1_dim, 1)
        self.fc_lower_binary_classifier = nn.Linear(fc1_dim, 1)
    ...
    def forward(self, input, input_feats=None):
        ...
        upper_class_logit = self.fc_upper_binary_classifier(x) if self.classify_garment else None
        lower_class_logit = self.fc_upper_binary_classifier(x) if self.classify_garment else None
