...
def predict_bedlam(pose_shape_model,
    ...
    preds_dict = {}
    crop_dir = 'output/crop/cat/'
    for img_name in os.listdir(crop_dir):
        img_path = os.path.join(crop_dir, img_name)
        seg_path = os.path.join(f'{img_path.replace("crop", "seg_npz").split(".")[0]}.npz')
        proxy_rep_segmaps = torch.from_numpy(np.flip(np.load(seg_path)['seg_maps'], axis=1).astype(bool)).unsqueeze(0).to(device)
        ...
        with torch.no_grad():
            ...
            #masks = np.stack([cv2.resize(cv2.imread(x), (256, 256)) for x in mask_paths])
            #masks = torch.from_numpy(masks).float().to(device) / 255.
            #proxy_rep_segmaps = ((masks[:, :, :, 0] + masks[:, :, :, 1] + masks[:, :, :, 2]) / 3).unsqueeze(0)
            #proxy_rep_segmaps = torch.swapaxes(proxy_rep_segmaps, 2, 3)
    ...
    np.savez('output/preds/npz/cat/preds.npz', **preds_dict)
